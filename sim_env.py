...
def make_sim_env(task_name):
    ...
    if task_name == "sim_transfer_cube_scripted":
        ...
        xml_path = os.path.join(XML_DIR, f"viperx_transfer_cube.xml")
        physics = mujoco.Physics.from_xml_path(xml_path)
        task = SingleArmTransferCubeTask(random=False)
        env = control.Environment(
            physics, task, time_limit=20, control_timestep=DT, n_sub_steps=None, flat_observation=False
        )
    else:
...
class SingleArmTransferCubeTask(base.Task):
    ...
    def initialize_episode(self, physics):
        ...
        with physics.reset_context():
            physics.named.data.qpos[:8] = START_ARM_POSE_SINGLE_ARM
            np.copyto(physics.data.ctrl, START_ARM_POSE_SINGLE_ARM)
    ...
    def get_reward(self, physics):
        # return whether  is holding the box
        all_contact_pairs = []
...
def test_sim_teleop():
    ...
    master_bot_left = InterbotixManipulatorXS(
        robot_model="wx250s", group_name="arm", gripper_name="gripper", robot_name=f"master_left", init_node=True
    )
    master_bot_right = InterbotixManipulatorXS(
        robot_model="wx250s", group_name="arm", gripper_name="gripper", robot_name=f"master_right", init_node=False
    )
